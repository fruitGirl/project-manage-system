 ...
 * @Last Modified time: 2019-11-05 19:09:49
 */
<template>
  <div v-loading="testLoading || caseLoading" class="project-test-sheet-detail">
    ...
    <pm-button-group
      :show-divide="true"
      :buttons="buttons"
      @back="$PMRouter.push('/project/test-sheet-list')"
      @startTest="handleStartTest"
      @editor="
        $PMRouter.push({
          path: '/project/editor-test',
      ...
      @bugList="$PMRouter.push('/project/bug-list')"
      @report="
        $PMRouter.push({
    ...
    <finish-dialog
      :show.sync="isVisible"
      :type="dialogType"
      :gmt-estimate-end="gmtEstimateTime"
...
import KitTable from '@/components/business/project/test-sheet-detail/kit-table.vue';
...
export default {
  ...
  methods: {
    ...
    ...rootMapActions('project/test-sheet-list', [
      'finishTestForm',
      'closeTestForm'
    ]),
    dateFormat,
    ...
    // 开始测试
    handleStartTest() {
      this.gmtEstimateTime = this.testFormDetail.gmtEstimateStart;
      this.dialogType = 2;
      this.isVisible = true;
    },
    ...
    // 完成测试
    handelFinishTest() {
      this.gmtEstimateTime = this.testFormDetail.gmtEstimateEnd;
      this.dialogType = 1;
      this.isVisible = true;
    },
