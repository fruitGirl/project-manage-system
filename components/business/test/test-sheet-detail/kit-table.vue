    ...
    <el-button
      ...
      @click="
        $PMRouter.push({
          ...
          params: {
            url: `/test/test-sheet-detail?id=${$route.query.id}`
          }
        })
      "
    >
      ...
      <template v-slot:title="{ rowData }">
        <div>
          <pm-link
            target="_blank"
            :to="
              `/test/use-case-detail?testCaseId=${rowData.id}
`
            "
...
import BugListDialog from '@/components/business/test/test-sheet-detail/bug-list-dialog.vue';
import {
  SETTINGS,
  COLUMN,
  SORT_CONFIG
