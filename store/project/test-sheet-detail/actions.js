import services from '@/services/project/test-sheet-detail';
...
export default {
  ...
  async startTestForm({ commit, dispatch, state }, payload) {
    try {
      await services.startTestFormApi(payload);
    } catch (err) {
  ...
  async getTestFormCasePage({ commit, dispatch, state }, payload) {
    try {
      ...
      const data = await services.getTestFormCasePageApi(payload);
  ...
  async getTestFormDetail({ commit, dispatch, state }, payload) {
    try {
      ...
      const data = await services.getTestFormDetailApi(payload);
      const {
        testForm = {},
        allTestCase,
        executedTestCaseIds,
  ...
  async removeTestFormCase({ commit, dispatch, state }, payload) {
    try {
      await services.removeTestFormCaseApi(payload);
  ...
  async initTestCaseBug({ commit, dispatch, state }, payload) {
    try {
      ...
      const data = await services.initTestCaseBugApi({ testCaseId: id });
  ...
  async getBugsList({ dispatch, commit, state }, payload) {
    const {
      bugs = [],
    } = await services.getBugQueryBySourceApi(payload);
