import service from '@/services/project/test-sheet-list';
...
export default {
  ...
  async getTestFormPageQuery({ commit, dispatch, state }, payload) {
    try {
      ...
      const data = await service.testFormPageQueryApi(payload);
  ...
  async finishTestForm({ commit }, payload) {
    try {
      await service.finishTestFormApi(payload);
  ...
  async closeTestForm({ commit }, payload) {
    try {
      await service.closeTestFormApi(payload);
  ...
  async getBugsList({ dispatch, commit, state }, payload) {
    const {
      bugs = []
    } = await service.getTestFormBugLinkApi(payload);
